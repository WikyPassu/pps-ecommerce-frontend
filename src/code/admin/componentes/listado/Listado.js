...
function getFilas(atributos = [], datos = [], attrKey, onEditClick, onDeleteClick, onShowClick,attrFuncs) {
  return datos.map((dato) => <tr key={"tr" + dato[attrKey]}>
    ...
    {onEditClick&&<td style={{textAlign: "center"}} key={`td${dato[attrKey]}btnEditar`}><Button key={`${dato[attrKey]}btnEditar`} onClick={()=>{onEditClick(dato)}}>Editar</Button></td>}
    {onDeleteClick&&<td style={{textAlign: "center"}} key={`td${dato[attrKey]}btnEliminar`}><Button key={`${dato[attrKey]}btnEliminar`} onClick={()=>{if(window.confirm("¿Está seguro que quiere eliminar este elemento?")){onDeleteClick(dato)}}}>Eliminar</Button></td>}
    {onShowClick&&<td style={{textAlign: "center"}} key={`td${dato[attrKey]}btnMostrar`}><Button key={`${dato[attrKey]}btnMostrar`} onClick={()=>{onShowClick(dato)}}>Ver</Button></td>}
  </tr>)
}
